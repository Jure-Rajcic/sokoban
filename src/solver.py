...
def solveSokoban(method, layout):
...
def solve_levels(input_path, output_path):
    # Read the levels from the input JSON file
    ...
    for level in levels:
        ...
        level['Solution'] = solution  # Append the solution to the level data
        solved_levels.append(level)
        print('Level', len(solved_levels), 'solved in', time_str, 'solution: ', solution)    
    # Write the solved levels to the output JSON file
    with open(output_path, 'w') as file:
        json.dump(solved_levels, file, indent=4)
